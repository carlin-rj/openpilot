...
from openpilot.frogpilot.common.frogpilot_utilities import delete_file, extract_tar, get_folder_size, load_json_file, run_cmd, update_json_file
...
class ModelManager:
      ...
      if version in {"v1", "v2", "v3", "v4", "v5", "v6"}:
        ...
        if not needs_download:
          for suffix in ["driving_policy", "driving_vision"]:
            ...
            local_size = self.model_sizes.get(onnx_filename)
  ...
  def check_tinygrad(self, boot_run, repo_url):
    if not TINYGRAD_REPO_PATH.exists():
      ...
      if boot_run:
        ...
        current_size = (
          get_folder_size(TINYGRAD_PATH, excluded_patterns)
          + get_folder_size(TINYGRAD_MODELD_PATH, excluded_patterns)
          + get_folder_size(TINYGRAD_REPO_PATH, excluded_patterns)
        )
      ...
      local_size = next(iter(self.tinygrad_size.values()), 0)
  ...
  def compile_model(self, model_name, sm):
    ...
    for suffix in ["driving_policy", "driving_vision"]:
      ...
      run_cmd(compile_command, "Model compiled successfully!", "Failed to compile the model...", env=env)
      ...
      run_cmd(metadata_command, f"Successfully extracted metadata from {onnx_path.name}!", f"Failed to extract metadata from {onnx_path.name}...")
  ...
  def fetch_all_model_sizes(self, repo_url):
    ...
    try:
      if is_github:
        ...
        api_url = f"https://gitlab.com/api/v4/projects/{repo_encoded}/repository/tree?ref=Models"
      else:
      ...
      if is_github:
        for file in model_files:
          model_sizes[file["name"]] = file.get("size", 0)
      else:
        for file in model_files:
  ...
  def update_tinygrad(self):
    ...
    try:
      ...
      sizes = {tinygrad_tar_path.name: tinygrad_tar_path.stat().st_size}
      update_json_file(self.tinygrad_size_path, sizes)
      print(f"Updated size for {tinygrad_tar_path.name} in {self.tinygrad_size_path.name}")
