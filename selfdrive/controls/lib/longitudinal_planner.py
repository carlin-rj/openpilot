...
class LongitudinalPlanner:
  ...
  def update(self, sm, classic_longitudinal, frogpilot_toggles):
    self.mode = 'blended' if (sm['controlsState'].experimentalMode or sm['frogpilotPlan'].experimentalMode) else 'acc'
    ...
    self.mpc.update(sm['radarState'], sm['frogpilotRadarState'], v_cruise, x, v, a, j, sm['frogpilotPlan'].tFollow, sm['frogpilotPlan'].trackingLead, frogpilot_toggles, personality=sm['controlsState'].personality)
    ...
    output_a_target_mpc, output_should_stop_mpc = get_accel_from_plan(self.v_desired_trajectory.tolist(), self.a_desired_trajectory.tolist(),
                                                                      frogpilot_toggles.longitudinalActuatorDelay, frogpilot_toggles.vEgoStopping)
  ...
  def publish(self, sm, pm):
    ...
    longitudinalPlan.hasLead = sm['frogpilotRadarState'].leadOne.status
